...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<List<PostResponse>>> getUserPosts(@PathVariable String username, @AuthenticationPrincipal CustomUserDetails userDetails) {
        List<PostResponse> response = postService.findByUsername(username, userDetails.getId());
        return ResponseEntity.ok(ApiResponse.success(response));
    }

    @PostMapping("/{username}/follow")
    public ResponseEntity<ApiResponse<FollowResponse>> follow(@PathVariable String username, @AuthenticationPrincipal CustomUserDetails userDetails) {
        FollowResponse response = followService.follow(username, userDetails.getId());
        return ResponseEntity.ok(ApiResponse.success(response));
    }
    @DeleteMapping("/{username}/follow")
    public ResponseEntity<ApiResponse<FollowResponse>> unfollow(@PathVariable String username, @AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    public ResponseEntity<ApiResponse<List<UserResponse>>> getFollowers(@PathVariable String username) {
        List<UserResponse> responses = followService.getFollowers(username);
    ...
    public ResponseEntity<ApiResponse<List<UserResponse>>> getFollowings(@PathVariable String username) {
        List<UserResponse> responses = followService.getFollowings(username);
        return ResponseEntity.ok(ApiResponse.success(responses));
    }
}
